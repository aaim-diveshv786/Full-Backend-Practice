...
const generateRefreshAndAccessToken = async (userId)=>{
        try{
            ...
            throw new ApiError(500 , 'Error while generating tokens 1');
        }
    }
const registerUser = asyncHandler(async(req , res)=>{
    ...
    // }
    //we can use some modern or advance method for checking the field in one go
...
const getUserChannelProfile = asyncHandler(async(req , res)=>{
    ...
    const channel = await User.aggregate([  // piplines
    {
        $match:{
            username:username?.toLowerCase()
        }
...
const getWatchHistory = asyncHandler(async(req , res)=>{
    ...
    .status(200)
    .json(
        new ApiResponse(200 , user[0].watchHistory , "Watch history has been fectched")
    )
})
export  {registerUser,
    loginUser, 
